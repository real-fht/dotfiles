...
---@module 'daemons.hardware.network'
---------------------------------------------------------------------------------
...
function network:connect_to_access_point(access_point, password, auto_connect)
  ...
  if #connections == 0 then
    ...
    end, { call_id = "awesomewm-network-daemon" }, profile, access_point.device_proxy_path, access_point.path)
  else
    -- connections[1]:Update(profile)
    for _, connection in pairs(connections) do
      -- Deleting then readding the connection since the Update method doesn't work.
      -- The UpdateUnsaved then followed by a Save doesn't work too.
      connection:Delete()
    end
    self._private.client_proxy:AddAndActivateConnectionAsync(function(proxy, context, success, failure)
      if failure ~= nil then
...
local function new()
  ...
  client_properties_proxy:connect_signal(function(self, interface, data)
